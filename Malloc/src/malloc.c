...
//内存池(4字节对齐)__align(4) __align(4)__no_init__no_init  
#pragma pack(push) //保存对齐状态 
#pragma pack(4)//设定为4字节对齐  
uint8_t mem1base[MEM1_MAX_SIZE];									//内部SRAM内存池
//uint8_t mem2base[MEM2_MAX_SIZE] @(0X68000000+MEM2_MAX_SIZE);		//外部SRAM内存池
#pragma pack(pop)//恢复对齐状态
//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
//uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] @(0X68000000+MEM2_MAX_SIZE);	//外部SRAM内存池MAP
//内存管理参数	   
